...
# load dataset
dataframe = pandas.read_csv("train/in.tsv", sep='\t', header=None)
dataset = dataframe.values
# split into input (X) and output (Y) variables
X = pandas.DataFrame(dataframe, columns=[1,2,5])
Y = dataset[:,0]
...
dataset = dataframe.values
# split into input (X) and output (Y) variables
X_dev = pandas.DataFrame(dataframe, columns=[0,1,4])
...
dataset = dataframe.values
# split into input (X) and output (Y) variables
X_test = pandas.DataFrame(dataframe, columns=[0,1,4])
